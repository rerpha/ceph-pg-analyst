...
with open(argv[1]) as pfg:  # uses the second argument in the command line
    pf = list(map(eval, list(map(str.strip, pfg.readlines()))))
...
hosts = [x for x in tree['nodes'] if True if x['type'] == 'host' else False]
...
val = list(pg_per_host.values())  # sets val to a list of the values in pg_per_host
mean = numpy.mean(val)
...
hist_opt = input("would you like a histogram of the hosts per pg?(y/n)")
